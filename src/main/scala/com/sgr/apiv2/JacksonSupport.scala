...
/**
 * @author sunguorui
 * @date 2022年07月06日 10:29 上午
 */
object JacksonSupport extends JacksonSupport {
  def getJsonNode(content: String)(
    implicit objectMapper: ObjectMapper = Jackson.defaultObjectMapper): JsonNode =
    objectMapper.readTree(content)

  def isError(content: String)(
    implicit objectMapper: ObjectMapper = Jackson.defaultObjectMapper): Boolean =
    !isSuccess(content)

  def isSuccess(content: String)(
...
trait JacksonSupport {
  ...
  : FromEntityUnmarshaller[A] =
    jsonStringUnmarshaller.map(data =>
      objectMapper.readValue(data, Jackson.typeReference[A]).asInstanceOf[A])
